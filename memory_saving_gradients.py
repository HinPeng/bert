...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
    ...
    # debug_recomp_tensor(ts_filtered, checkpoints)
...
def debug_recomp_tensor(ts, checkpoints):
    recomps = format_ops(ts)
    tmp = format_ops(checkpoints)
    ...
    with open("./result_70.txt", "w") as fin:
        fin.writelines(["CHECKPOINTS:\n"])
        fin.writelines(tmp)
        fin.writelines(["RECOMP TENSORS\n"])
        fin.writelines(recomps)
        fin.writelines(["ERROR TENSORS\n"])
        fin.writelines(error)
    # for tensor in formatted_args:
    #     print("DEBUG recomp tensor: %s" % tensor)
    # # debug_print("recomps tensor: %s", recomps)
    # debug_print("error tensor: %s", error)
